...
import com.allever.android.sample.customview.linecharview.LineDataSet;
...
public class LineChartView extends View {
    ...
    private Paint mLineTextPaint;//横线,坐标画笔
    private Paint mBezierPaint; //贝塞尔曲线画笔
    private Paint mVerticalPaint;//竖直线画笔
    private Paint mRectPaint;//标注矩形
    private Paint mTextPaint;//标注文字画笔

    private float mHeight;
    private float mWidth;

    private float mDownX = -1f;//按下时x坐标

    private Context mContext;

    private final float mMarginLeftRight;//左右边界值,
    private final float mMarginTopBottom;//上下边界值

    private List<LineDataSet> mLineDataSetList = new ArrayList<>();//数据源.存放曲线参数
    private List<String> mXNameList = new ArrayList<>();//横坐标时间(总数据)格式01:00,由外部设置
    private List<String> mXNameListShow = new ArrayList<>();//显示的横坐标 时间
    ...
    private final int mBezierPointCount = 11;//选择N个点绘制曲线~~ N-1段曲线
