...
public class EasyPickerView extends View {
    ...
    //中间文字颜色，默认Color.BLACK
    private int cetenrColor;
    // 文字之间的间隔，默认10dp
    ...
    public EasyPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        cetenrColor = a.getColor(R.styleable.EasyPickerView_epvCenterColor, Color.BLACK);
    ...
    public void isRecycle(boolean recycle) {
        if (recycle) {
            isRecycleMode = true;
        } else {
            isRecycleMode = false;
        ...
        if (null != dataList && dataList.size() > 0) {
            ...
            for (int i = -half; i <= half; i++) {
                ...
                if (index >= 0 && index < size) {
                    ...
                    tempScale = tempScale < 1.0f ? 0.6f : tempScale;
...
//                    Log.i("tempY:" + tempY, "cy" + cy);
                    Shader shader1 = new LinearGradient(0, 0, 0, 60, Color.parseColor("#93e7f3"), Color.parseColor("#2baed4"), Shader.TileMode.MIRROR);
                    Shader shader2 = new LinearGradient(0, 0, 0, 60, Color.parseColor("#C7C8C8"), Color.parseColor("#C7C8C8"), Shader.TileMode.MIRROR);
                    if (tempY == cy) {
                        textPaint.setShader(shader1);
                    } else {
    ...
    public void moveTo(int index) {
        ...
        int dy = 0;
        int centerPadding = textHeight + textPadding;
        Log.i("Pucker", "textHeight:" + textHeight + "--textPadding:" + textPadding + "--centerPadding:" + centerPadding);
        if (!isRecycleMode) {
            dy = (curIndex - index) * centerPadding;
            Log.i("Pucker", "dy:" + dy + "--curIndex:" + curIndex + "--centerPadding:" + centerPadding);
        } else {
